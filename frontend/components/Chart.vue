	...
	<div :id="show_options ? 'outer_wrapper' : ''" class="full_size">
		<div :class="show_options ? 'fullscreen_wrapper' : 'wrapper'" :id="show_options ? 'popup' : ''">
			...
			<div class="chart_wrapper">
				...
				<h5>{{ options.title }}</h5>
				<div :class="show_options ? 'fullscreen_chart' : 'chart'" v-if="options.chart_type == 'pie' && Object.keys(chart_data).length == 1 && show_chart">
					<ChartPie
						:pie="chart_data"
						:key="key"
					/>
				</div>
				<div :class="show_options ? 'fullscreen_chart' : 'chart'" v-if="options.chart_type == 'line' && Object.keys(chart_data).length == 1 && show_chart">
					<ChartLine
						:line="chart_data"
						:key="key"
					/>
