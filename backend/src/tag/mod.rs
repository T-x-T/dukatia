...
impl Tag {
	async fn is_valid_parent(&self, pool: &Pool) -> bool {
		...
		if TagLoader::new(pool).set_filter_id(self.parent_id.unwrap()).get_first().await.is_err() {
			return false;
		}
		...
		loop {
			...
			let next_tag = TagLoader::new(pool).set_filter_id(next_parent_id_to_check).get_first().await;
			if next_tag.is_err() {
				break;
			}
...
#[derive(Debug, Clone)]
pub struct DeepTagLoader<'a> {
	pool: &'a Pool,
	query_parameters: QueryParameters,
}

impl<'a> Loader<'a, DeepTag> for DeepTagLoader<'a> {
	fn new(pool: &'a Pool) -> Self {
		Self {
			pool,
			query_parameters: QueryParameters::default(),
	...
	async fn get(self) -> Result<Vec<DeepTag>, Box<dyn Error>> {
		return db::DeepTagDbReader::new(self.pool)
			.set_query_parameters(self.query_parameters)
			.execute()
			.await;
	}
